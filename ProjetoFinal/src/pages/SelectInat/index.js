...
export default function SelectInat({ navigation }) {
        ...
        return(
            <View>
                <View style={styles.card}>
                        ...
                        <Text style={styles.nome} onPress={()=>{navigation.navigate('DetalheIn',{ 
      id: item.id, 
    ...
    const getClientes = ()=>{
        db.transaction(txn => {
            txn.executeSql(
                `SELECT * FROM clientes where status = false`,
                [],
                (sql,res)=> {
    ...
    const buscar = ()=>{
        db.transaction(txn => {
            txn.executeSql(
                `SELECT * FROM clientes where status =false and nome like'${busca}%'`,
                [],
                (sql,res)=> {
                    console.log("Dados selecionados com exito")
                    let len = res.rows.length;
    ...
    const atualizar = ()=>{
        navigation.navigate('SelectInat')
    }
    ...
    return (
            ...
            <View style={styles.all}>
                <View style={styles.header}>
                    <Text style={styles.title}>Clientes Inativos</Text>
                </View>

                <Text style={styles.subt}
                    onPress={() => { navigation.navigate('Selects') }}
                >Visualizar clientes ativos</Text>
